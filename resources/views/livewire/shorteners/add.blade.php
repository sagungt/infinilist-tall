<div class="flex flex-col min-h-screen py-12 bg-gray-50">
    <div class="container mx-auto w-full md:w-[80%] lg:w-1/2">

        <h1 class="text-4xl font-extrabold my-10">Create New Shortener</h1>
        ...
        <div class="flex flex-col">
            <form action="{{ route('shortener.store', ['kind' => 'post']) }}" method="post" class="w-full" x-data="data">
                @csrf

                <div class="flex flex-col items-start mb-6" x-data="{ url: rand(5) }">
                ...
                <div class="mb-6" x-data="{ selectedType: 'post' }" x-init="getList(selectedType)" x-effect="getList(selectedType)">
                    ...
                    <div class="flex">
                        ...
                        <div id="dropdown-states" class="z-10 hidden bg-white divide-y divide-gray-100 rounded shadow w-44">
                            <ul class="py-1 text-sm text-gray-700" aria-labelledby="states-button">
                                ...
                                <li>
                                    <button x-on:click="selectedType = 'external'" type="button" class="inline-flex w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                        <div class="inline-flex items-center">
                                            External
                                        </div>
                                    </button>
                        ...
                        <template x-if="['post', 'chapter'].includes(selectedType)">
                            <div class="w-full">
                                <label for="states" class="sr-only">Choose a target</label>
                                <select class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-r-lg border-l-gray-100 dark:border-l-gray-700 border-l-2 focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" name="parent_id">
                                    <option selected>Choose a target</option>
                                    <template x-for="item in data">
                                        <option x-bind:value="item.id" x-text="selectedType === 'post' ? item.title : item.name"></option>
                                    </template>
                                </select>
                            </div>
                        </template>
