<div x-data="{ categories: dropdownCategories() }" class="flex flex-col min-h-screen py-12 bg-gray-50">
    <div class="container mx-auto mt-8 w-full md:w-[80%] lg:w-2/3">
        <a href="{{ route('profile.post.list') }}" class="text-sm px-2 py-1 border border-slate-500 rounded-full font-bold">Back</a>
        <h1 class="text-4xl font-extrabold mt-5 mb-10">Edit Post</h1>
        ...
        <div class="flex flex-col">
            ...
            {{-- @dd($post['title']) --}}
            <form action="{{ route('post.update', ['slug' => $post['slug']]) }}" method="post" class="w-full" enctype="multipart/form-data">
                @csrf

                <div class="mb-6">
                    <label for="title" class="block mb-2 text-sm font-medium text-gray-900">Title</label>
                    <input type="text" id="title" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 bg-white" placeholder="Post title" name="title" value="{{ old('title', $post['title']) }}">
                ...
                <div class="mb-6">
                    <label for="prev_categories" class="block mb-2 text-sm font-medium text-gray-900">Previous Categories</label>
                    <div class="flex">
                        @foreach ($post['categories'] as $category)
                            <div x-data="{ el: $el }" class="flex w-fit justify-center items-center m-1 font-medium py-1 px-1 bg-white rounde border">
                                <div class="text-xs font-normal leading-none max-w-full flex-initial w-fit">{{ $category['name'] }}</div>
                                <div class="flex flex-auto flex-row-reverse">
                                    <div x-on:click="removePreviousCategory(el, @js($category['id']))">
                ...
                <div class="mb-6">
                    <label for="prev_categories" class="block mb-2 text-sm font-medium text-gray-900">Previous Tags</label>
                    <div class="flex">
                        @foreach ($post['tags'] as $tag)
                            <div x-data="{ el: $el }" class="flex justify-center items-center m-1 font-medium py-1 px-1 bg-white rounde border">
                                <div class="text-xs font-normal leading-none max-w-full flex-initial">{{ $tag['name'] }}</div>
                                <div class="flex flex-auto flex-row-reverse">
                                    <div x-on:click="removePreviousTag(el, @js($tag['id']))">
                                        <svg class="fill-current h-4 w-4 " role="button" viewBox="0 0 20 20">
                                        <path d="M14.348,14.849c-0.469,0.469-1.229,0.469-1.697,0L10,11.819l-2.651,3.029c-0.469,0.469-1.229,0.469-1.697,0
                                            c-0.469-0.469-0.469-1.229,0-1.697l2.758-3.15L5.651,6.849c-0.469-0.469-0.469-1.228,0-1.697s1.228-0.469,1.697,0L10,8.183
                                            l2.651-3.031c0.469-0.469,1.228-0.469,1.697,0s0.469,1.229,0,1.697l-2.758,3.152l2.758,3.15
                ...
                <div id="hidden-input-categories" class="hidden">
                    @foreach ($post['categories'] as $category)
                        <input id="hidden-category-{{$category['id']}}" type="hidden" name="categories[]" value="{{ $category['id'] }}">
                    @endforeach
                    @foreach ($post['tags'] as $tag)
                        <input id="hidden-tag-{{$tag['id']}}" type="hidden" name="tags[]" value="{{ $tag['id'] }}">
                    @endforeach
                </div>

                <div class="mb-6" x-data="categoryData" x-init="await getCategories()">
                                            ...
                                            <div x-show.transition.origin.top="isOpen()" class="absolute shadow top-100 bg-white z-40 w-full left-0 rounded max-h-select" x-on:click.away="close">
                                                <div class="flex flex-col w-full overflow-y-auto h-64">
                                                    <template x-for="(option, index) in options" :key="index" class="overflow-auto">
                                                        <div class="cursor-pointer w-full border-gray-100 rounded-t border-b hover:bg-gray-100" x-on:click="select(index, $event)">
                ...
                <div class="mb-6" x-data="tagData" x-init="await getTags()">
                    <template x-if="loaded">
                        <div>
                ...
                <div class="mb-6">
                    <img src="{{ $post['cover']['path'] }}" alt="{{ $post['title'] }}" class="w-full h-40 object-cover rounded-lg">
                </div>

                <div class="mb-6">
                    <label class="block mb-2 text-sm font-medium text-gray-900" for="cover">Cover</label>
                    <input type="file" id="cover" name="cover" class="text-sm text-grey-500 file:mr-5 file:py-2 file:px-6 border border-gray-300 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-blue-50 file:text-slate-700 hover:file:cursor-pointer hover:file:bg-slate-300 hover:file:text-slate-700 w-full bg-white p-1.5 rounded-lg " />
                ...
                <div class="mb-6">
                    ...
                    <textarea id="content" rows="4" class="block p-2.5 w-full max-w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 no-tailwindcss-base" placeholder="Your post here..." name="content">{{ old('content', $post['content']) }}</textarea>
                ...
                <div class="flex items-start mb-6">
                    <div class="flex items-center h-5">
                        <input id="is_published" @if($post['is_published']) checked @endif type="checkbox" class="w-4 h-4 bg-gray-50 rounded border border-gray-300 focus:ring-3 focus:ring-blue-300" name="is_published">
                    </div>
...
@push('scripts')
    <script>
        var myEditor;
        var prevCategories = @js(collect($post['categories'])->map(fn ($c) => $c['id']));
        var prevTags = @js(collect($post['tags'])->map(fn ($t) => $t['id']));
        ...
        function dropdownCategories() {
            ...
            select(index, event) {
                if (prevCategories.includes(Number(this.options[index].value))) return;
        ...
        function dropdownTags() {
            ...
            isOpen() { return this.show === true },
            select(index, event) {
                if (prevTags.includes(Number(this.options[index].value))) return;
                if (!this.options[index].selected) {
                    this.options[index].selected = true;
                    this.options[index].element = event.target;
                    this.selected.push(index);
        ...
        function removePreviousCategory(el, id) {
            el.remove();
            removeInputCategory(id);
            prevCategories = prevCategories.filter((v) => v !== id);
        }
        function removePreviousTag(el, id) {
            el.remove();
            removeInputTag(id);
            prevTags = prevTags.filter((v) => v !== id);
        }
    </script>
@endpush
